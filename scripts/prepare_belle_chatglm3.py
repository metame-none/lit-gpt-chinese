...
DESTINATION_PATH = Path("data/belle_chat_ramdon_10k_chatglm3")
...
class SupervisedDataset():
    ...
    def __init__(
        ...
        self.user_tokens = [tokenizer.token_to_id("<|user|>")]
        self.assistant_tokens = [tokenizer.token_to_id("<|assistant|>")]
        self.ignore_index = IGNORE_INDEX

    def preprocessing(self, example):
        input_ids = []
        for token in ["[gMASK]", "sop"]:
            input_ids.append(self.tokenizer.token_to_id(token))
        labels = [self.ignore_index] * 2
        ...
        for message in example["conversations"]:
            from_ = message["from"]
            value = message["value"]
            value_ids = self.tokenizer.encode(value, return_tensor=False)

            if from_ == "human":
                new_tokens = self.user_tokens + newline_token + value_ids
                input_ids += new_tokens
                labels += [self.ignore_index] * (len(new_tokens))
            else:
                input_ids += self.assistant_tokens + newline_token + value_ids
                labels += self.assistant_tokens + newline_token + value_ids
